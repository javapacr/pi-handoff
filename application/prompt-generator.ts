...
export async function generateHandoffPrompt(
	...
	const initialLine = `Generating handoff prompt · context: ${formatK(input.conversationText.length)}`;

	// Statuses go to the caller's `report` sink and — when a standalone loader
	// exists — also update the loader line.
	const runGeneration = async (
		loader: HandoffLoader | undefined,
	): Promise<HandoffPromptResult> => {
		...
		if (settings?.model && handoffModel.id !== settings.model) {
			...
			status(`fallback: ${handoffModel.id}`);
		}

		status(initialLine);
		status(
			`Generating with ${handoffModel.provider}/${handoffModel.id} (effort: ${effort ?? "off"})`,
		);
		...
		for (const model of modelsToTry) {
			try {
				...
				const text = await generateWithModel(
					...
					effort,
					(p) => {
						// Throttle streaming updates to ≥250ms apart.
						const now = Date.now();
						if (now - lastProgressAt < 250) return;
						lastProgressAt = now;
						const size =
							p.phase === "writing" ? `${formatK(p.chars)} chars` : formatK(p.chars);
						status(
							`Generating with ${model.provider}/${model.id} · ${p.phase}… ${size}`,
						);
					},
				);
	...
	const handleFailure = (err: unknown): HandoffPromptResult => {
	...
	// Executor-driven mode: the caller owns the loader UI and receives phase
	// lines via `report`; generation runs headless here.
	if (report) {
		return runGeneration(undefined).catch(handleFailure);
	}

	return ctx.ui.custom<HandoffPromptResult>((tui, theme, _kb, done) => {
		const loader = new HandoffLoader(tui, theme, initialLine, () =>
			done({ prompt: null }),
		);

		runGeneration(loader).then(done).catch((err) => done(handleFailure(err)));
